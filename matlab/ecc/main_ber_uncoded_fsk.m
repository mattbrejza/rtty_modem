...
% Section 1
%===============================================================
function main_ber_uncoded_fsk( SNR_start, SNR_delta, SNR_stop, bits_per_symbol)
%===============================================================
...
if ischar(SNR_delta)
    ...
    filename = ['results_uncoded_fsk_M=',num2str(bits_per_symbol),'_',num2str(SNR_start),'_',num2str(seed),'.mat'];
        ...
        while SNR <= SNR_stop
            ...
            if flag    
                ...
                while bit_count < 1000000% || error_counts(iteration_count) < 10
                    ...
                    sym_count = ceil(frame_length/bits_per_symbol);
                    order = 2^(bits_per_symbol);
                    symbols = ceil(order*rand(1,sym_count));
                    
                    % FSK modulate them
                    tx = zeros([2^(bits_per_symbol),sym_count]);
                    for i=1:sym_count
                        tx(symbols(i),i) = 1;
                    end
                    ...
                    % Generate some noise
                    N0 = 1/(10^(SNR/10));
                    noise = sqrt(N0/2)*(randn(order,sym_count)+1i*randn(order,sym_count));

                     %add wobble
                    %u = 1:length(a_tx);
                    %u=u./50;
                    %tx = tx.*0.2.*(sin(1.2*u)+.7*sin(2.1*u)+0.5*sin(3*u)+2.5);
                    ...
                    % FSK demodulator
                    [~,sym_out] = max(rx,[],1);
                   % a_c = abs(a_rx1)-abs(a_rx2);
                   

                    % Make a hard decision and see how many bit errors we have.
                    %errors = sum((a_c > 0) ~= a);
                    errors_s = sum(sym_out ~= symbols);
                    errors = sum(sum(dec2bin(sym_out-1,bits_per_symbol) ~= dec2bin(symbols-1,bits_per_symbol)));

                    bit_count = bit_count + sym_count*bits_per_symbol;
                    ...
                    if size(results,1) < SNR_count
                        ...
                        results(SNR_count,2) = sym_count*bits_per_symbol;
                        ...
                        results(SNR_count,1) = SNR;
                        results(SNR_count,2) = results(SNR_count,2)+sym_count*bits_per_symbol;
