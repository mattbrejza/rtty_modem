...
% Section 1
%===============================================================
function main_ber(frame_length, SNR_start, SNR_delta, SNR_stop, rate)
%===============================================================
...
rng(1);
    ...
    filename = ['results_',num2str(frame_length),'_',num2str(SNR_start),'_',num2str(rate),'_',num2str(seed),'.mat'];
    ...
    while(keepgoing)
        ...
        while SNR <= SNR_stop
            ...
            if size(results,1) < SNR_count
                flag = 1;
            else if results(SNR_count,iteration_count+2) < 500
                    flag = 1;
                end
            end
...
%                     f_tx = f_tx.*0.2.*(sin(1.2*u)+.7*sin(2.1*u)+0.5*sin(3*u)+2.5);
                    ...
                    while chance < chances && iteration_index <= iteration_count
                        ...
                        % Perform decoding.
                        % These labels match those used in Figure 2.11 of Liang Li's nine month report.
                        y_e = component_decoder_(y_a,c_c);
                        ...
                        % Perform decoding.
                        % These labels match those used in Figure 2.11 of Liang Li's nine month report.
                        z_e = component_decoder_(z_a,d_c);
