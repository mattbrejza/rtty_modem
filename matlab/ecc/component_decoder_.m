...
function extrinsic_uncoded_llrs = component_decoder(apriori_uncoded_llrs, apriori_encoded_llrs)
    ...
    for bit_index = 2:length(apriori_uncoded_llrs)
       for transition_index = 1:size(transitions,1)
           alphas(transitions(transition_index,2),bit_index) = maxstar([alphas(transitions(transition_index,2),bit_index),alphas(transitions(transition_index,1),bit_index-1) + uncoded_gammas(transition_index, bit_index-1) + encoded_gammas(transition_index, bit_index-1)]);   
       end
    end
    ...
    for bit_index = 1:length(apriori_uncoded_llrs)    
       ...
       for transition_index = 1:size(transitions,1)
           if transitions(transition_index,3)==0
               prob0 = maxstar([prob0, deltas(transition_index,bit_index)]);
           else
               prob1 = maxstar([prob1, deltas(transition_index,bit_index)]);
           end      
       end
