function [ out ] = genbpsk( samples, sample_rate, sequence, baud_rate, centre, SNR, bits, stops )
...
N     = 10;   % Order
Fpass = 1.5.*baud_rate;  % Passband Frequency
...
for i=1:samples
 %   integ = integ + (out(i)/sample_rate);
    out(i) = sin(2*pi*(centre*t(i)))*outq(i);
end
